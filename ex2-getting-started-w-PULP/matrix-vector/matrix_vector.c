...
void stop_perf_counter()
{
	...
	*/
	...
	float cpi = (double) cycles_counter / instr_counter;
	// MAC/Cycles
	float mac_on_cycles = (double) N*M / cycles_counter;
	// Instructions/Cycles
	float instructions_on_cycles = (double) instr_counter / cycles_counter;
	// Instructions/MAC
	float instructions_on_mac = (double) instr_counter / N*M;
