  ...
  */
  ...
  for (int h_tile_idx = 0; h_tile_idx < nb_h_tile; h_tile_idx++) {
    for (int w_tile_idx = 0; w_tile_idx < nb_w_tile; w_tile_idx++) {
      ...
      /* compute next buffer idx -- either 0 or 1*/
      // const int next_buffer_idx = <complete>
      
      /* compute next w tile idx -- either 0 or 1*/
      // const int next_w_tile_idx = (w_tile_idx + 1) % nb_w_tile;
      
      /* compute next h tile idx -- either 0 or 1*/
      /* note: If the next_w_tile is 0, that means we went into a new row, so the next_h_tile is h_tile + 1*/
      // const int next_h_tile_idx = <complete> 

      /* counter of number of tiles processed so far*/
      // const int next_tile_idx = <complete>
